...
const TopNav = ({ username, isBanker }) => {
  return (
    // <div className="flex flex-row items-center justify-evenly bg-[#FEF3F3]  bg-opacity-90 w-full h-14 m-0 pl-2 pr-4 shadow-lg">
    <div
      className={`flex flex-row items-center justify-evenly bg-opacity-90 w-full h-14 m-0 pl-2 pr-4 shadow-lg ${
        isBanker ? "bg-blue-200" : "bg-[#FEF3F3]"
      }`}
    >
      <Logo />
      {/* <HashtagIcon /> */}
      ...
      {/* <Search />
      <BellIcon /> */}
      <UserCircle username={username} isBanker={isBanker} />
    </div>
  );
};

const ButtonLink = ({ text }) => {
  const { signinOIDCSSO } = useAuth();
...
const Title = () => (
  <h3
    className="ext-base text-black tracking-wider font-bold text-opacity-80 
...
const Logo = () => (
  <a
    href="https://www.114bank.co.jp/"
    className="flex items-center space-x-3  bg-white bg-opacity-0 rtl:space-x-reverse"
  >
    <img
...
const Divider = () => (
  <hr className="bg-[#B30000] dark: bg-gray-800 border border-[#B30000] dark:border-gray-800 rounded-full my-2 mx-2" />
);

const UserCircle = ({ username, isBanker }) => {
  const [isOpen, setIsOpen] = useState(false);
  const navigate = useNavigate();
  const { signoutUser, signoutSSO } = useAuth();
      ...
      {isOpen && (
        <div className="absolute  right-0 mt-4 w-48 bg-[#EEECE1] shadow-md rounded-md">
          ...
          <div className="block text-start  p-2">
            <span
              className="flex text-[#A6121F] hover:bg-[#FEF3F3] rounded-md"
              onClick={signOut}
            >
              <div className="flex space-x-6  items-start justify-start ">
